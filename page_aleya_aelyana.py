...
def render():
    ...
    # ------------------------------------------
    # FIX TYPO & SET ORDERING
    # ------------------------------------------
    # Membetulkan typo terus dalam dataframe sebelum diproses untuk graf
    if "GPA" in df.columns:
        df["GPA"] = df["GPA"].replace("2.50 - 2.49", "2.50 - 2.99")
    if "CGPA" in df.columns:
        df["CGPA"] = df["CGPA"].replace("2.50 - 2.49", "2.50 - 2.99")

    # Susunan akademik dan GPA yang konsisten
    ...
    # Susunan GPA dari rendah ke tinggi
    gpa_order = ["Below 2.00", "2.00 - 2.49", "2.50 - 2.99", "3.00 - 3.69", "3.70 - 4.00"]
...
* **GPA 2.50 - 2.99** group (corrected label) show highest spread and maximum scores, indicating that this group experiences the strongest insomnia symptoms.
...
* Managing insomnia is a key factor in academic success. Students with the best grades tend to maintain the healthiest sleep profiles.
    ...
    if {"Insomnia_Category", "DaytimeFatigue"}.issubset(df.columns):
        fat_table = pd.crosstab(df["Insomnia_Category"], df["DaytimeFatigue"], dropna=False)
        fat_melted = fat_table.reset_index().melt(
            id_vars="Insomnia_Category", var_name="DaytimeFatigue", value_name="Count"
        )

        fig_d = px.bar(
            fat_melted, x="Insomnia_Category", y="Count", color="DaytimeFatigue",
    ...
    if {"Insomnia_Category", "AcademicPerformance"}.issubset(df.columns):
        ...
        fig_e.update_layout(yaxis=dict(autorange="reversed"), showlegend=False)
    ...
    # f) Heatmap
    # ------------------------------------------
    st.subheader("Figure C6 - Correlation Heatmap")
    ...
    if len(existing_cols) >= 2:
        ...
        fig_f = px.imshow(
            corr_matrix, text_auto=".2f", color_continuous_scale=SUNSET,
            zmin=-1, zmax=1, title="Correlation Heatmap: Sleep vs Academic Outcomes",
        )
